...
def piece_wise_lt():
    ...
    # img = cv2.imread("../../img/spine.tif", cv2.IMREAD_GRAYSCALE)
    img = cv2.imread("../../img/pollen_washedout.tif", cv2.IMREAD_GRAYSCALE)

    img = cv2.resize(img, (256, 256), 0, 0, cv2.INTER_LINEAR)  # Original image but rescaled to 256x256
    img2 = np.copy(img)  # Copy of image
    hist = np.copy(img)
    hist2 = np.copy(img)
    T0 = 255 * np.ones(img.shape, np.uint8)  # Array 256x256 de valores 255

    cv2.namedWindow("Transformation", cv2.WINDOW_AUTOSIZE)

    # Points (r1, s1) and (r2, s2) used to for contrast stretching
    # These points go from 0 to 255, because they're intensity values
    # Refer to Figure 3.10 in DIP by Gonzalez and Woods
    r1 = 65
    s1 = 65
    r2 = 195
    s2 = 195

    # Bars to select the points coordinates
    ...
    while True:
        ...
        # Here we select the points values
        ...
        T = np.copy(T0)  # Remember that T0 is an array of 256x256 filled with 255
        p1 = (r1, T.shape[1] - 1 - s1)  # First point location
        p2 = (r2, T.shape[1] - 1 - s2)  # Second point location
        # Draw a line in T from (0, ...) to p1 with color (0, 0, 0), thickness 2, type LINE_8 and 0 shift
        cv2.line(T, (0, T.shape[0] - 1), p1, (0, 0, 0), 2, cv2.LINE_8, 0)
        # Draw a circle in T with center=p1 and radius=4, color=0, thickness=2, lineType=LINE_8 and shift=0
...
def thresholding_2():
    ...
    # img = cv2.imread("../../img/breast_little.tif", cv2.IMREAD_GRAYSCALE)
    img = cv2.imread("../../img/kidney.tif", cv2.IMREAD_GRAYSCALE)
    img2 = np.copy(img)
...
def highlight_spec_r_int():
    ...
    # 0 - THRESH_BINARY means that values less than 'thresh' are set to 0, otherwise 1
    # 1 - THRESH_BINARY_INV means that values higher than 'thresh' are set to 0, otherwise 1
    # 2 - THRESH_TRUNC means that values higher than 'thresh' are to truncated to it
    # 3 - THRESH_TOZERO means that values less than 'thresh' are set to 0
    # 4 - THRESH_TOZERO_INV means that values higher than 'thresh' are set to 0
...
def bit_slicing():
    ...
    # Slice = 7 shows most significant bit plane
    # ...
    # Slice = 0 shows less significant bit plane
    slice = 7
...
def local_histogram_processing():
    ...
    # wsize is subtracted from img.shape because otherwise would give index error
...
# Local Histogram Processing 2
def local_histogram_processing_2():
    # See Figure 3.27
    ...
    for x in range(wsize, img.shape[0] - wsize):
        for y in range(wsize, img.shape[1] - wsize):
            ...
            intensity = img[x, y]
            intensity_new = E*intensity if ((avg_local <= k0*avg_global) and
                                            (k1*std_global <= std_local) and
                                            (std_local <= k2*std_global)) else intensity
...
if __name__ == '__main__':
    ...
    thresholding_1()
    thresholding_2()
    highlight_spec_r_int()
    bit_slicing()
    histogram_equalization()
    local_histogram_processing()
    local_histogram_processing_2()
